...
pub const EMPTY: Token = Token(usize::MAX);
...
impl Wheel {
    ...
    pub fn new(builder: &Builder) -> Wheel {
        ...
        eprintln!("num_slots = {:?}", num_slots);
        eprintln!("builder.get_initial_capacity() = {:?}", builder.get_initial_capacity());
        eprintln!("Instant::now() = {:?}", Instant::now());
        eprintln!("builder.get_max_capacity() = {:?}", builder.get_max_capacity());
        eprintln!("builder.get_tick_duration() = {:?}", builder.get_tick_duration());
        eprintln!("mask = {:?}", mask);
        Wheel {
...
//        let wheel = vec![Slot { head: EMPTY, next_timeout: None }; num_slots];
//        wheel.iter().zip(self.wheel.iter())
//            .for_each(|(a, b)| if a != b {
//                println!("{:?}",b);
//            });
